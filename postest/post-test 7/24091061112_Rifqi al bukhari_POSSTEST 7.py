...
def hapus_pesanan():
    ...
    if  hapus in data_pesanan:
        ...
        if memastikan_hapus == "1":
            ...
            print(f"pesanan {hapus} telah dihapus!")
            print(" ")
            return hapus
...
def hitung_total_pembelian(data_pesanan, total=0, index=0):
    if index == len(data_pesanan):
        print(f"Harga total yang harus dibayar: {total}")
        return total
    nama = list(data_pesanan.keys())[index]
    order = data_pesanan[nama]

    total += order['harga']
    return hitung_total_pembelian(data_pesanan, total, index + 1)

def total_harga():
    total=0
    for order in data_pesanan.values():
        total += order['harga']
        return total
                ...
                while True:
                    ...
                    if memilih == "1":
                        ...
                        if not data_pesanan:
                            ...
                            total_harga = hitung_total_pembelian(data_pesanan)
